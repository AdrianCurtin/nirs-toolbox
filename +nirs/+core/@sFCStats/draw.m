...
if(nargin>7 && ~isempty(TypesShown))
...
if(length(obj)>1)
    cond=vertcat(obj.conditions);
    uconds = unique(cond, 'stable');
else
    uconds = unique(obj.conditions, 'stable');
end

if(nargin>6 && ~isempty(ConditionsShown))
...
for cIdx=1:length(uconds)
    tbl=obj.table;
    tbl(~ismember(tbl.condition,uconds{cIdx}),:)=[];
    ...
    utypesOrigin={utypesOrigin{ismember(utypesOrigin,utypes)}};
    utypesDest={utypesDest{ismember(utypesDest,utypes)}};
