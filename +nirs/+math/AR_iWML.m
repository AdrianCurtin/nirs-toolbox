function stats = AR_iWML(d,X,Pmax,tune)
...
%         Xfiltered=[];
    ...
    for i = 1:nChan
        ...
        while norm(B-B0)/norm(B0) > 1e-2 && iter < maxiter
            ...
            % perform IRLS
            [B, S,lambda] = nirs.math.robust_REML(Xf(lstValid,:),yf(lstValid),{speye(length(lstValid),length(lstValid))},...
                Qp,lambda,'bisquare',tune,'off');
