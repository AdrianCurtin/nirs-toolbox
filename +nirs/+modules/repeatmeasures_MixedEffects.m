classdef repeatmeasures_MixedEffects < nirs.modules.AbstractModule
    ...
    methods
        function obj = repeatmeasures_MixedEffects( prevJob )
        ...
        function G = runThis( obj, S )
            ...
            if(~isROI)
                for i=1:height(vars)
                    ...
                    lst=find(sd.source==vars.source(i) & sd.detector==vars.detector(i) &...
                        ismember(sd.type,vars.type(i)));
                    SDlabel{i,1}=num2str(lst);
                    
                end
                vars.source=[]; vars.detector=[]; vars.file_idx=[]; vars.type=[];
                vars.SDlabel=SDlabel;
            else
                for i=1:height(vars)
                    
                    lst=find(ismember(sd.ROI,vars.ROI(i)) &...
                        ismember(sd.type,vars.type(i)));
                    SDlabel{i,1}=num2str(lst);
                    
                end
                vars.ROI=[]; vars.file_idx=[]; vars.type=[];
                vars.SDlabel=SDlabel;
            end
            ...
            try
                lm1 = fitrm([table(beta,'VariableNames',{respvar}) tmp], obj.formula);
